...
function carregarProgresso() {
    botoesDias.forEach(botao => {
        ...
        const status = localStorage.getItem(`cardio-${dia}`);
    ...
    let semanaSalva = localStorage.getItem('cardio-semana-atual');
    
    if (!semanaSalva) {
        semanaSalva = 1;
        localStorage.setItem('cardio-semana-atual', 1);
    } else {
...
function atualizarTextoSemana(numeroSemana) {
    if (numeroSemana === 1) {
        textoSemana.innerText = "1ª Semana de Cardio";
    } else {
        const semanaPassada = numeroSemana - 1;
        textoSemana.innerText = `${semanaPassada}ª semana de cardio concluída!`;
    }
}
...
botoesDias.forEach(botao => {
    botao.addEventListener('click', () => {
        ...
        if (botao.classList.contains('concluido')) {
            localStorage.setItem(`cardio-${dia}`, 'concluido');
        } else {
            localStorage.removeItem(`cardio-${dia}`);
        }
    });
});
...
btnReset.addEventListener('click', () => {
    if (confirm('Você finalizou todos os treinos e quer avançar para a PRÓXIMA SEMANA?')) {
        ...
        let semanaAtual = Number(localStorage.getItem('cardio-semana-atual')) || 1;
        ...
        localStorage.setItem('cardio-semana-atual', semanaAtual);

        atualizarTextoSemana(semanaAtual);

        // ✨ EXIBE O MODAL COM A MENSAGEM PERSONALIZADA
        textoParabens.innerText = `Sensacional! A sua ${semanaConcluida}ª semana de cardio foi totalmente concluída. Continue firme rumo à próxima!`;
