...
function carregarProgresso() {
    botoesDias.forEach(botao => {
        ...
        const status = localStorage.getItem(`cross-${dia}`);
    ...
    let semanaSalva = localStorage.getItem('cross-semana-atual');
    
    if (!semanaSalva) {
        semanaSalva = 1;
        localStorage.setItem('cross-semana-atual', 1);
    } else {
...
function atualizarTextoSemana(numeroSemana) {
    if (numeroSemana === 1) {
        textoSemana.innerText = "1ª Semana de Crossfit";
    } else {
        const semanaPassada = numeroSemana - 1;
        textoSemana.innerText = `${semanaPassada}ª semana de crossfit concluída!`;
    }
}
...
botoesDias.forEach(botao => {
    botao.addEventListener('click', () => {
        ...
        if (botao.classList.contains('concluido')) {
            localStorage.setItem(`cross-${dia}`, 'concluido');
        } else {
            localStorage.removeItem(`cross-${dia}`);
        }
    });
});
...
btnReset.addEventListener('click', () => {
    if (confirm('Você finalizou todos os treinos e quer avançar para a PRÓXIMA SEMANA?')) {
        ...
        let semanaAtual = Number(localStorage.getItem('cross-semana-atual')) || 1;
        
        const semanaConcluida = semanaAtual; 
        
        semanaAtual = semanaAtual + 1;
        localStorage.setItem('cross-semana-atual', semanaAtual);

        atualizarTextoSemana(semanaAtual);

        // ✨ EXIBE O MODAL COM A MENSAGEM PERSONALIZADA
        textoParabens.innerText = `Sensacional! A sua ${semanaConcluida}ª semana de WODs intensos foi totalmente concluída. O Box está orgulhoso!`;
