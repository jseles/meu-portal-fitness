...
function carregarProgresso() {
    botoesDias.forEach(botao => {
        ...
        const status = localStorage.getItem(`musculo-${dia}`);
    ...
    let semanaSalva = localStorage.getItem('musculo-semana-atual');
    
    if (!semanaSalva) {
        semanaSalva = 1;
        localStorage.setItem('musculo-semana-atual', 1);
    } else {
...
function atualizarTextoSemana(numeroSemana) {
    if (numeroSemana === 1) {
        textoSemana.innerText = "1ª Semana de Musculação";
    } else {
        const semanaPassada = numeroSemana - 1;
        textoSemana.innerText = `${semanaPassada}ª semana de musculação concluída!`;
    }
}
...
botoesDias.forEach(botao => {
    botao.addEventListener('click', () => {
        ...
        if (botao.classList.contains('concluido')) {
            localStorage.setItem(`musculo-${dia}`, 'concluido');
        } else {
            localStorage.removeItem(`musculo-${dia}`);
        }
    });
});
...
btnReset.addEventListener('click', () => {
    if (confirm('Você finalizou todos os treinos e quer avançar para a PRÓXIMA SEMANA?')) {
        ...
        let semanaAtual = Number(localStorage.getItem('musculo-semana-atual')) || 1;
        
        // 🎯 Guarda qual foi a semana que o usuário acabou de CONCLUIR para exibir no parabéns
        const semanaConcluida = semanaAtual; 
        
        semanaAtual = semanaAtual + 1;
        localStorage.setItem('musculo-semana-atual', semanaAtual);

        atualizarTextoSemana(semanaAtual);

        // ✨ EXIBE O MODAL COM A MENSAGEM PERSONALIZADA
        textoParabens.innerText = `Sensacional! A sua ${semanaConcluida}ª semana de musculação foi totalmente concluída. Continue progredindo carga rumo à próxima!`;
